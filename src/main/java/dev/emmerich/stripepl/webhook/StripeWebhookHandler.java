...
public class StripeWebhookHandler implements HttpHandler {
    ...
    public StripeWebhookHandler(JavaPlugin plugin, String webhookSecret, Map<String, List<String>> productCommands) {
        this.plugin = plugin;
                    ...
                    if (dataObject.isPresent() && dataObject.get() instanceof Session) {
                        ...
                        if (minecraftUsername != null) {
                            ...
                            for (LineItem lineItem : lineItems.getData()) {
                                ...
                                if (productCommands.containsKey(productId)) {
                                    List<String> commands = productCommands.get(productId);
                                    
                                    for (String cmd : commands) {
                                        String finalCommand = cmd.replace("{player}", minecraftUsername);
                                        plugin.getLogger().info("Dispatching command: " + finalCommand);
                                        // Schedule the command to run on the main server thread
                                        plugin.getServer().getScheduler().runTask(plugin, () -> plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), finalCommand));
                                    }
                                } else {
                                    plugin.getLogger().warning("No commands configured for product ID: " + productId);
                                }
                            }
